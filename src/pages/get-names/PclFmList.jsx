...
export default function PclFmList() {
  ...
  //Extracting selected pcl details
  const pclName = pcl.leader_name || "No PCL assigned";
  const pcsName = pcl.general_name || "No PCS assigned";

  const [searchTerm, setSearchTerm] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10;

  // Filter fm based on search term
  const filteredFm = fm.filter((fm) =>
    fm.member_name
      ? fm.member_name.toLowerCase().includes(searchTerm.toLowerCase())
      : false
  );

  // Calculate total pages for filtered fm
  const totalPages = Math.ceil(filteredFm.length / itemsPerPage);

  // Get current page's fm
  ...
  return (
    <div className="inline-block bg-white p-6 rounded shadow-md w-[900px] h-[680px] space-y-8">
      ...
      <div className="flex flex-row justify-between items-center text-sm">
        ...
        <span className="font-bold uppercase text-red-500">PCL: {pclName}</span>
            ...
            {currentFm.length > 0 ? (
              currentFm.map((fm, index) => (
                <tr key={index} className="even:bg-gray-50">
                  ...
                  <td className="border border-gray-300 p-2">{fm.purok}</td>
